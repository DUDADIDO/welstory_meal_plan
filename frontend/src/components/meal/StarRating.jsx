export default function StarRating({ mealName, rating, disabled, onRate }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-2 gap-y-1 border-t border-[var(--theme-border)] pt-4">
      <div className="flex" role="group" aria-label={`${mealName} 별점`}>
        {[1, 2, 3, 4, 5].map((star) => (
          <button
            ...
            onClick={() => onRate(star)}
            className={`px-0.5 text-[1.15rem] leading-none transition hover:scale-110 disabled:opacity-45 ${star <= selected ? 'text-[#ff9f0a]' : 'text-[var(--theme-rating-empty)] hover:text-[#ff9f0a]/70'}`}
          >★</button>
        ))}
      </div>
